...
#define Z_GSM_CHANNEL_P(zv)     ((gsmChannel *)((char *)(Z_OBJ_P(zv)) - XtOffsetOf(gsmChannel, std)))

#define GSM_SAMPLE_RATE          8000U
#define GSM_SAMPLES_PER_FRAME    160U
#define GSM_PCM_BYTES_PER_FRAME  320U
#define GSM_BYTES_PER_FRAME      33U
#define GSM_FRAME_DURATION_MS    20U
#define GSM_MAX_FRAMES_PER_CALL  50U
...
static zend_class_entry *gsm_ce;
static zend_object_handlers gsm_handlers;
...
static void gsm_channel_close(gsmChannel *obj) {
    if (obj->encoder) {
        gsm_destroy(obj->encoder);
        obj->encoder = NULL;
    }
    if (obj->decoder) {
        gsm_destroy(obj->decoder);
        obj->decoder = NULL;
    }
}

static zend_object *gsm_create_object(zend_class_entry *ce) {
    gsmChannel *obj = zend_object_alloc(sizeof(gsmChannel), ce);

    obj->encoder = gsm_create();
    obj->decoder = gsm_create();

    zend_object_std_init(&obj->std, ce);
    object_properties_init(&obj->std, ce);
    obj->std.handlers = &gsm_handlers;

    return &obj->std;
}

static void gsm_free_object(zend_object *object) {
    gsmChannel *obj = (gsmChannel *) ((char *) object - XtOffsetOf(gsmChannel, std));

    gsm_channel_close(obj);
    zend_object_std_dtor(&obj->std);
}
...
ZEND_METHOD(gsmChannel, __construct) {}

ZEND_METHOD(gsmChannel, encode) {
    zend_string *input;
    ...
    const size_t len = ZSTR_LEN(input);
    if (len == 0 || (len % GSM_PCM_BYTES_PER_FRAME) != 0) {
        RETURN_FALSE;
    }

    const size_t frames = len / GSM_PCM_BYTES_PER_FRAME;
    if (frames > GSM_MAX_FRAMES_PER_CALL || frames > (SIZE_MAX / GSM_BYTES_PER_FRAME)) {
        RETURN_FALSE;
    }

    gsmChannel *self = Z_GSM_CHANNEL_P(getThis());
    if (!self->encoder) {
        RETURN_FALSE;
    }

    const size_t out_len = frames * GSM_BYTES_PER_FRAME;
    zend_string *out = zend_string_alloc(out_len, 0);
    gsm_signal *src = (gsm_signal *) ZSTR_VAL(input);
    gsm_byte *dst = (gsm_byte *) ZSTR_VAL(out);

    for (size_t i = 0; i < frames; i++) {
        gsm_encode(
            self->encoder,
            src + (i * GSM_SAMPLES_PER_FRAME),
            dst + (i * GSM_BYTES_PER_FRAME)
        );
    }
...
ZEND_METHOD(gsmChannel, decode) {
    ...
    const size_t len = ZSTR_LEN(input);
    if (len == 0 || (len % GSM_BYTES_PER_FRAME) != 0) {
        RETURN_FALSE;
    }

    const size_t frames = len / GSM_BYTES_PER_FRAME;
    if (frames > GSM_MAX_FRAMES_PER_CALL || frames > (SIZE_MAX / GSM_PCM_BYTES_PER_FRAME)) {
        RETURN_FALSE;
    }
    ...
    const size_t out_len = frames * GSM_PCM_BYTES_PER_FRAME;
    zend_string *out = zend_string_alloc(out_len, 0);
    gsm_byte *src = (gsm_byte *) ZSTR_VAL(input);
    gsm_signal *dst = (gsm_signal *) ZSTR_VAL(out);

    for (size_t i = 0; i < frames; i++) {
        if (gsm_decode(
                self->decoder,
                src + (i * GSM_BYTES_PER_FRAME),
                dst + (i * GSM_SAMPLES_PER_FRAME)
            ) != 0) {
...
ZEND_METHOD(gsmChannel, info) {
    gsmChannel *self = Z_GSM_CHANNEL_P(getThis());

    array_init(return_value);
    add_assoc_bool(return_value, "encoder_initialized", self->encoder != NULL);
    add_assoc_bool(return_value, "decoder_initialized", self->decoder != NULL);
    add_assoc_long(return_value, "sample_rate", GSM_SAMPLE_RATE);
    add_assoc_long(return_value, "samples_per_frame", GSM_SAMPLES_PER_FRAME);
    add_assoc_long(return_value, "pcm_bytes_per_frame", GSM_PCM_BYTES_PER_FRAME);
    add_assoc_long(return_value, "gsm_bytes_per_frame", GSM_BYTES_PER_FRAME);
    add_assoc_long(return_value, "frame_duration_ms", GSM_FRAME_DURATION_MS);
}

ZEND_METHOD(gsmChannel, close) {
    gsmChannel *self = Z_GSM_CHANNEL_P(getThis());

    gsm_channel_close(self);
    RETURN_TRUE;
}
...
ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_gsm_info, 0, 0, IS_ARRAY, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_gsm_close, 0, 0, _IS_BOOL, 0)
ZEND_END_ARG_INFO()

static const zend_function_entry gsm_methods[] = {
    ZEND_ME(gsmChannel, __construct, arginfo_gsm_construct, ZEND_ACC_PUBLIC | ZEND_ACC_CTOR)
    ZEND_ME(gsmChannel, encode,      arginfo_gsm_codec_io,  ZEND_ACC_PUBLIC)
    ZEND_ME(gsmChannel, decode,      arginfo_gsm_codec_io,  ZEND_ACC_PUBLIC)
    ZEND_ME(gsmChannel, info,        arginfo_gsm_info,      ZEND_ACC_PUBLIC)
    ZEND_ME(gsmChannel, close,       arginfo_gsm_close,     ZEND_ACC_PUBLIC)
    ZEND_FE_END
};

PHP_MINIT_FUNCTION(bcg729) {
    zend_class_entry ce, gsm_class_entry;
    ...
    INIT_CLASS_ENTRY(gsm_class_entry, "gsmChannel", gsm_methods);
    gsm_ce = zend_register_internal_class(&gsm_class_entry);
    gsm_ce->create_object = gsm_create_object;

    memcpy(&gsm_handlers, zend_get_std_object_handlers(), sizeof(zend_object_handlers));
    gsm_handlers.offset = XtOffsetOf(gsmChannel, std);
    gsm_handlers.free_obj = gsm_free_object;
